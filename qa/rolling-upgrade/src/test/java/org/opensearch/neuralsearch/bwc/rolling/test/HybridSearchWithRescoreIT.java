...
public class HybridSearchWithRescoreIT extends AbstractRollingUpgradeTestCase {
        ...
        switch (getClusterType()) {
            ...
            case MIXED:
                ...
                if (isFirstMixedRound()) {
                    ...
                    validateTestIndexOnUpgrade(totalDocsCountMixed, hybridQueryBuilder, rescorer);
                    addDocument(getIndexNameForTest(), "1", TEST_FIELD, TEXT_MIXED, null, null);
                } else {
                    totalDocsCountMixed = 2 * NUM_DOCS_PER_ROUND;
                    HybridQueryBuilder hybridQueryBuilder = getQueryBuilder(modelId, null, null);
                    validateTestIndexOnUpgrade(totalDocsCountMixed, hybridQueryBuilder, null);
                }
                break;
            case UPGRADED:
                try {
                    ...
                    HybridQueryBuilder hybridQueryBuilder = getQueryBuilder(modelId, null, null);
                    QueryBuilder rescorer = QueryBuilders.matchQuery(TEST_FIELD, RESCORE_QUERY).boost(0.3f);
                    validateTestIndexOnUpgrade(totalDocsCountUpgraded, hybridQueryBuilder, rescorer);
    ...
    private void validateTestIndexOnUpgrade(final int numberOfDocs, HybridQueryBuilder hybridQueryBuilder, QueryBuilder rescorer)
        throws Exception {
