...
public class BucketAggregationsWithHybridQueryIT extends BaseAggregationsWithHybridQueryIT {
    ...
    @SneakyThrows
    public void testEmptyResults_whenHybridQueryResultOrAggregationsEmptyResults_thenSuccessful() {
        updateClusterSettings(CLUSTER_SETTING_CONCURRENT_SEGMENT_SEARCH, false);
        testSumNestedIntoFiltersWithEmptyResults();
    }

    @SneakyThrows
    public void testEmptyResultsWithConcurrentSegmentSearch_whenHybridQueryResultOrAggregationsEmptyResults_thenSuccessful() {
        updateClusterSettings(CLUSTER_SETTING_CONCURRENT_SEGMENT_SEARCH, true);
        testSumNestedIntoFiltersWithEmptyResults();
    }
    ...
    private void testSumNestedIntoFiltersWithEmptyResults() throws IOException {
        try {
            prepareResources(TEST_MULTI_DOC_INDEX_WITH_TEXT_AND_INT_MULTIPLE_SHARDS, SEARCH_PIPELINE);
            // test case when hybrid query return empty results
            AggregationBuilder aggsBuilder = AggregationBuilders.filters(
                GENERIC_AGGREGATION_NAME,
                QueryBuilders.rangeQuery(INTEGER_FIELD_1).lte(3000),
                QueryBuilders.termQuery(KEYWORD_FIELD_1, KEYWORD_FIELD_1_VALUE)
            ...
            hybridQueryBuilder.add(QueryBuilders.termQuery(TEST_TEXT_FIELD_NAME_1, TEST_QUERY_TEXT2));
            ...
            for (Map<String, Object> bucket : buckets) {
                ...
                assertEquals(0, bucket.get(BUCKET_AGG_DOC_COUNT_FIELD));
                assertEquals(0.0, getAggregationValue(bucket, SUM_AGGREGATION_NAME), DELTA_FOR_SCORE_ASSERTION);
            }

            // test case when hybrid query return some results but aggregation filtering out everything
            AggregationBuilder aggsBuilderNoResults = AggregationBuilders.filters(
                GENERIC_AGGREGATION_NAME,
                QueryBuilders.rangeQuery(INTEGER_FIELD_1).gte(3000),
                QueryBuilders.termQuery(KEYWORD_FIELD_1, KEYWORD_FIELD_1_VALUE)
            ).otherBucket(true).subAggregation(AggregationBuilders.sum(SUM_AGGREGATION_NAME).field(INTEGER_FIELD_1));

            Map<String, Object> searchResponseAsMapEmptyResultAggs = executeQueryAndGetAggsResults(
                List.of(aggsBuilderNoResults),
                hybridQueryBuilder,
                TEST_MULTI_DOC_INDEX_WITH_TEXT_AND_INT_MULTIPLE_SHARDS,
                0
            );

            Map<String, Object> aggregationsEmptyResult = getAggregations(searchResponseAsMap);
            assertNotNull(aggregations);

            List<Map<String, Object>> bucketsEmptyResult = getAggregationBuckets(aggregationsEmptyResult, GENERIC_AGGREGATION_NAME);
            assertNotNull(bucketsEmptyResult);
            assertEquals(3, bucketsEmptyResult.size());

            for (Map<String, Object> bucket : bucketsEmptyResult) {
                assertEquals(2, bucket.size());
                assertEquals(0, bucket.get(BUCKET_AGG_DOC_COUNT_FIELD));
                assertEquals(0.0, getAggregationValue(bucket, SUM_AGGREGATION_NAME), DELTA_FOR_SCORE_ASSERTION);
            }
    ...
    private void testAdjacencyMatrixAggsEmptyResults() throws IOException {
        try {
            prepareResources(TEST_MULTI_DOC_INDEX_WITH_TEXT_AND_INT_MULTIPLE_SHARDS, SEARCH_PIPELINE);
    ...
    protected Map<String, Object> executeQueryAndGetAggsResults(
        final List<Object> aggsBuilders,
        QueryBuilder queryBuilder,
        String indexName
    ) {
