...
public final class MultiLeafFieldComparator implements LeafFieldComparator {
    ...
    // Returns: any N < 0 if the doc's value is sorted after the bottom entry (not competitive), any N > 0 if the doc's value is sorted
    // before the bottom entry and 0 if they are equal.
    @Override
    ...
    // Params: doc – that was hit
    // Returns: any N < 0 if the doc's value is sorted after the top entry (not competitive),any N > 0 if the doc's value is sorted before
    // the top entry and 0 if they are equal.
    @Override
    ...
    // Params: slot – which slot to copy the hit to doc – docID relative to current reader
    @Override
    ...
    // Returns: an iterator over competitive docs that are stronger than already collected docs or null if such an iterator is not available
    // for the current comparator or segment.
    @Override
