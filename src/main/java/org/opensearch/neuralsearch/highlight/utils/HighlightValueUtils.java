...
public final class HighlightValueUtils {
     ...
     * @return separator-joined string, empty when no elements
     */
    public static String joinElements(final List<String> elements) {
        if (elements == null || elements.isEmpty()) return "";
        return String.join(ELEMENT_SEPARATOR, elements);
    }
    ...
    public static List<String> applyHighlightsPerElement(
        ...
        // Element boundaries in joined coordinates, using ELEMENT_SEPARATOR.
        ...
        for (String element : elements) {
            ...
            pos = end + ELEMENT_SEPARATOR.length();
        }
        ...
        for (int index = 0; index < elements.size(); index++) {
            ...
            for (int i = spanIndex; i < spans.size() && spans.get(i)[0] < elementEnd; i++) {
                ...
                // Skip spans crossing element boundaries.
                if (span[0] < elementStart || span[1] > elementEnd) {
                    continue;
                }
                localHighlights.add(
                    Map.of(
