...
public class NeuralQueryValidationUtil {
    ...
    private static boolean isModelIdRequiredForNeuralQueryForKnn(final NeuralQueryBuildStage buildStage) {
        // During rewrite stage, model ID is always required
        if (NeuralQueryBuildStage.REWRITE.equals(buildStage)) {
            return true;
        }

        // During FROM_X_CONTENT(when buildStage is null we view it as the FROM_X_CONTENT stage), model ID is required if default model ID
        // isn't supported
        boolean isBuiltFromXContent = (buildStage == null || NeuralQueryBuildStage.FROM_X_CONTENT.equals(buildStage));
        boolean supportsDefaultModel = isClusterOnOrAfterMinReqVersionForDefaultDenseModelIdSupport();

        return isBuiltFromXContent && supportsDefaultModel == false;
    }
    ...
    public static void validateTargetFieldConfig(
        ...
        if (validationErrors.isEmpty() == false) {
            throw new IllegalArgumentException(
                String.format(
                    Locale.ROOT,
                    "Invalid neural query against the field %s. Errors: %s",
                    fieldName,
